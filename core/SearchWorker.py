...
from .Utils import normalize_audience_raw, viewer_may_read_audience
...
class SearchWorker:
    ...
    def _filter_results_by_audience(self, results: list, viewer_role: str | None) -> list:
        if not viewer_role or not results:
            return results
        out = []
        for hit in results:
            try:
                ...
                if viewer_may_read_audience(viewer_role, aud):
                    out.append(hit)
            except Exception:
                out.append(hit)
        return out

    def search(self, keyword, allowed_subsets=None, viewer_role=None):
        ...
        if not os.path.exists(cache_path):
            ...
            return self._filter_results_by_audience(
                self._slow_search(keyword, allowed_subsets), viewer_role
            )
        ...
        return self._filter_results_by_audience(results, viewer_role)
